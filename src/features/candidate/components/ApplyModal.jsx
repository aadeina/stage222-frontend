...
const ApplyModal = ({ isOpen, onClose, internship, onSuccess }) => {
    ...
    const navLinks = [
        { label: t('internshipDetail.home'), href: '/candidate/dashboard' },
        { label: t('internshipDetail.internships'), href: '/internships' },
        { label: t('internshipDetail.jobs'), href: '/jobs' },
        { label: t('internshipDetail.login'), href: '/login' },
        { label: t('internshipDetail.registerAsStudent'), href: '/register' },
        { label: t('internshipDetail.forEmployers'), href: '/recruiter/login' },
    ];
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl mx-2 relative flex flex-col max-h-[90vh]">
                ...
                <div className="flex items-center justify-between border-b p-4 bg-gradient-to-r from-[#00A55F] to-[#008c4f]">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-white font-bold text-lg">{t('internshipDetail.applyNow')}</span>
                    </div>
                ...
                <div className="px-4 py-3 border-b">
                    ...
                    <div className="text-xs text-gray-500">{internship?.location} • {t('internshipDetail.applyBy')} {internship?.application_deadline ? new Date(internship.application_deadline).toLocaleDateString() : ''}</div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-4 py-6 space-y-4">
                    ...
                    {success ? (
                        ...
                        <>
                            {questions.length > 0 ? (
                                <>
                                    <div className="text-gray-800 font-medium mb-2">{t('internshipDetail.screeningQuestions')}</div>
                                    {questions.map((q, idx) => (
                                        <div key={idx} className="mb-4">
                                            ...
                                            <textarea
                                                ...
                                                placeholder={t('internshipDetail.yourAnswer')}
                                            />
                                        </div>
                                    ))}
                                </>
                            ...
                            >
                                ...
                                {t('internshipDetail.submitApplication')}
                            </button>
                        </>
                    )}
                </form>
