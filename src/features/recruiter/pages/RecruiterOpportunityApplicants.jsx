                                ...
                                applicants.map((app, idx) => (
                                    <tr key={idx} className="hover:bg-gray-50 transition">
                                        <td className="px-4 py-4 whitespace-nowrap flex items-center gap-2">
                                            ...
                                            <span className="font-medium text-gray-900 truncate">{app.candidate_name || app.candidate?.full_name || app.candidate?.name || 'N/A'}</span>
                                        </td>
                                        <td className="px-4 py-4 whitespace-nowrap text-gray-700">
                                            <span className="flex items-center gap-1"><FaEnvelope className="h-4 w-4 text-gray-400" /> {app.candidate_email || app.candidate?.email || 'N/A'}</span>
                                        </td>
                                        <td className="px-4 py-4 whitespace-nowrap">
                                            <div className="flex items-center gap-2">
                                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${statusColors[app.status] || 'bg-gray-100 text-gray-600'}`}>{app.status}</span>
                                                <select
